...
public class RefreshLeagueListingTask implements Runnable{
    ...
    public void run() {
        ...
        liveGamesCache.values().forEach(gameAggregate -> {
        ...
        if(null != gameAggregate) {
            ...
            if (leagueAggregateOptional.isPresent()) {
                ...
                if (leagueAggregate.getLeagueInfo().getLeagueSeasonMap().containsKey(gameInfo.getSeason())) {
                    ...
                    populateSeason(gameInfo, leagueSeason);
    ...
    private void populateSeason(GameInfo gameInfo, LeagueSeason leagueSeason) {
        try {
            Season season = restTemplate.getForObject("http://new.core.espnuk.org/v2/sports/cricket/leagues/"+(gameInfo.getLeagueId()/13)+"/seasons/"+gameInfo.getSeason(), Season.class);
            ...
            log.error("Error while fetching season details league: {}, season: {}, massage: {}",gameInfo.getLeagueId(), gameInfo.getSeason(), e.getMessage());
        }
    }
