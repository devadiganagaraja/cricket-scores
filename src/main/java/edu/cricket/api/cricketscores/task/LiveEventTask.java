...
@Service
public class LiveEventTask {

    private static final Logger log = LoggerFactory.getLogger(LiveEventTask.class);
    ...
    public void refreshLiveEvents() {
        liveEvents.keySet().forEach(eventId ->{
            EventAggregate eventAggregate = liveEventsCache.get(eventId);
            if(null == eventAggregate){
                ...
                updateEventScore(eventAggregate.getEventInfo());
                updateEventScoreCard(eventAggregate);
                if(eventAggregate.getEventInfo().getInternationalClassId() > 0 || eventAggregate.getEventInfo().getGeneralClassId() > 0 ) {
                    liveEventsCache.put(eventId, eventAggregate);
